...
for INPUT_YAML in "$@"; do
    ...
    if [ -n "$BUILD_JOB_ID" ]; then
        DATA_JOB_ID=$(sbatch --dependency=afterok:$BUILD_JOB_ID --parsable scripts/data_generation_ptbc.slurm "$INPUT_YAML" "$BASE_OUTPUT_DIR")
    else
        DATA_JOB_ID=$(sbatch --parsable scripts/data_generation_ptbc.slurm "$INPUT_YAML" "$BASE_OUTPUT_DIR")
    fi
