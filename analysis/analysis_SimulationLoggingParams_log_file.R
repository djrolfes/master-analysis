...
analyze_deltaH_autocorr <- function(data, out_pdf_prefix, skip_steps = 0) {
  ...
  ts_df <- data %>% transmute(step = as.integer(as.numeric(.data[[step_col]])), delta_H = as.numeric(delta_H)) %>% filter(!is.na(step) & !is.na(delta_H))

  # Timeseries plot
  ts_pdf <- paste0(out_pdf_prefix, "_delta_H_timeseries.pdf")
  write_log(paste0("analyze_deltaH_autocorr: saving timeseries to ", ts_pdf))
  tryCatch({
    p <- ggplot(ts_df, aes(x = step, y = delta_H)) + geom_line(color = "darkgreen") + geom_point(size = 0.7, alpha = 0.6) + labs(title = "delta_H vs HMC step", x = "HMC step", y = "delta_H") + theme_minimal()
  ...
  uw_res <- tryCatch({ hadron::uwerrprimary(ac_series, pl = TRUE) }, error = function(e) {
